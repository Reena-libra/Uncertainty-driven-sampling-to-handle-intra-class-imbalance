...
def cal_loss(pred, gold, smoothing=False, ignore_index=255):
    ...
    if smoothing:
        ...
        # Convert 'gold' to LongTensor if it's not already
        gold = gold.long()

        # Ensure 'pred' is the correct type
        assert pred.dtype == torch.float32, "pred must be a FloatTensor"
    ...
    def mc_dropout_variance(self, model, inputs, cls_label, num_samples=5):
        ...
        with torch.no_grad():
            for i in range(num_samples):
                ...
                #print(cls_label.shape,inputs.shape)
